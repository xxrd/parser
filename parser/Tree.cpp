...
Tree::Node::Node(const std::string& name, const std::string& value, const int id) : name(name), value(value), id(id), childNodes(nullptr) {}

Tree::Node::Node(const std::string& name, list<Tree::Node*>* childNodes, const int id) : name(name), childNodes(childNodes), id(id) {}
...
std::ostream& operator<<(std::ostream& os, Tree& p) {
	p.output(os);
	return os;
}